...
# Add 'chrt' suffic to EAVE_LINKNO to distinguish it from the EAVE number of those
# who received a second dose when it comes to matcing.
z_chrt <- df_cohort %>%
  dplyr::rename(EAVE_LINKNO_v1 = EAVE_LINKNO) %>%
...
for(j in 1:(length(a_months)-1)){
  ...
  z_chrt_j <- z_chrt %>%
    left_join(any_hosp_month, by= c("EAVE_LINKNO_v1"= "EAVE_LINKNO")) %>%
  ...
  # Only include those who were first dose vaccinated before z_strt
...
limit_matches <- function(df_matches, limit){
  ...
  if (limit != 'inf'){
    ...
    saveRDS(df_matches, paste0("./data/df_matches_second_dose_", limit, '_', z_event_endpoint,".rds"))
  }
}
