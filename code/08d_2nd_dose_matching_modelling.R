...
##                 Rachel Mulholland <rachel.mulholland@ed.ac.uk> 
...
title(main=z_title, xlab="days from 2nd dose vaccination",ylab="cumulative risk")
...
plot(z, fun="event", col=c(1,2), conf.int=T)
title(main=z_title, 
      xlab="14 days from 2nd dose vaccination",ylab="cumulative risk")
legend("topleft",legend=levels(df_cc_ps_matches$vacc), lty=1, col=c(1,2), cex=0.8)
...
title(main=z_title, 
      xlab="days from 2nd dose vaccination",ylab="cumulative risk",
      sub = "PB")
legend("topleft",legend=levels(df_cc_ps_matches$vacc), lty=1, col=c(1,2), cex=0.8)
...
z_pois_pred_rr <- z_pois_pred %>%
  group_by(z.yr) %>%
  mutate(RR = est[vacc == "v2"]/est[vacc == "v1"]) %>%
  mutate(var1 = ((est[vacc == "v2"])^2)/((est[vacc == "v1"])^2)) %>%
  mutate(var2 = (var.fit[vacc == "v2"])/((est[vacc == "v2"])^2) + 
           (var.fit[vacc == "v1"])/((est[vacc == "v2"])^2)) %>%
